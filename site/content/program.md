---
numbers:
  - num: "227"
    title: entrepreneur·e·s recruté·e·s
    btnTitle: Découvrir les EIG
    btnUrl: /talents
  - num: "131"
    title: projets accompagnés
    btnTitle: Découvrir les projets
    btnUrl: /defis
  - num: "58"
    title: organismes partenaires
    btnTitle: Découvrir les sponsors
    btnUrl: /agents
steps:
  title: La trajectoire des EIG
  list:
    - icon: /img/1_valider_hypothese.svg
      step: Amorçage
      duration: 1 à 2 mois
      title: Cadrer et engager une dynamique
      dates: À l'arrivée de l'EIG
      desc: Le travail démarre par la compréhension du problème et du contexte. On
        précise ce qui doit être résolu et dans quelles conditions. On fait
        émerger une traction qui concrétise le potentiel.
    - icon: /img/2_prouver_le_concept.png
      step: Incubation
      duration: 12 à 18 mois
      title: Concevoir et éprouver la valeur
      dates: Pendant le contrat initial de l’EIG
      desc: Un premier service est conçu, testé et amélioré en continu. On le
        confronte aux usages pour vérifier son utilité réelle. On mesure la
        valeur créée et on ajuste le service en fonction.
    - dates: En cas de renouvellement
      desc: Le service est étendu. On en améliore la robustesse, on en élargit l’usage
        et on en optimise le déploiement. On consolide les conditions de son
        adoption à plus grande échelle.
      icon: /img/4_amplifier.png
      step: Accélération
      duration: 12 à 18 mois
      title: Déployer et renforcer le service
    - step: Ancrage
      duration: Durée indéterminée
      title: Faire durer et évoluer la proposition
      dates: Au-delà du contrat de l’EIG
      desc: Le service s’inscrit dans l’organisation publique. On en stabilise
        l’exploitation, la gouvernance et les modalités d’amélioration. On ancre
        sa valeur dans les pratiques et dans la durée.
      icon: /img/3_prouver_la_valeur.png
layout: program
url: /programme
image: img/bootcamp9_auto_x2-1-.jpg
title: Entreprendre pour le service public
desc: >-
  En 2016, à l’occasion du sommet mondial du Partenariat pour un Gouvernement
  Ouvert, la Présidence de la République lance le programme Entrepreneur·e·s
  d’intérêt général (EIG). Son ambition : développer une fonction publique plus
  ouverte et plus entrepreneuriale, dans ses méthodes comme dans sa culture.


  Aujourd’hui, au sein du réseau beta.gouv.fr, le programme mobilise une nouvelle génération d’entrepreneur·e·s pour faire émerger, déployer et porter à maturité des services et des plateformes au cœur des politiques publiques. Il accompagne des projets qui ont vocation à s’ancrer durablement dans l’action publique.
how:
  desc: >-
    ## Comment ça marche ?


    #### Pour les entrepreneur·e·s


    Vous rejoignez les Services du Premier ministre en contrat de projet d’un an, renouvelable, pour intervenir au sein d’une administration partenaire. Vous y pilotez un produit numérique : comprendre le besoin, structurer le travail, lancer le service, mesurer ses résultats et accompagner son déploiement.


    #### Pour les administrations


    Vous proposez un projet fondé sur un besoin avéré, au service d’un enjeu de politique publique. S’il est retenu, vous accueillez un·e EIG pour en piloter le lancement et le développement. Vous apportez le cadre, l’appui hiérarchique et les moyens nécessaires à sa réussite et à son inscription dans la durée.
then:
  desc: >-
    ## Et après ?


    Le programme vise à ancrer dans l’administration les compétences, les méthodes et la culture nécessaires au développement de services numériques au service d’enjeux majeurs de politique publique.


    Au-delà de chaque mission, les EIG contribuent à installer une approche fondée sur l’attention aux besoins réels, la mesure de la valeur créée et l’amélioration continue des services.
quote:
  photo: /img/marguerite-yourcenar3.jpg
  quote: Même là où j’innovais, j’aimais à me sentir avant tout un continuateur.
  name: Marguerite Yourcenar
  desc: Mémoires d’Hadrien
menu: program
---
